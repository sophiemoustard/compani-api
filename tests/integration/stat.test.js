...
describe('GET /stats/customer-fundings-monitoring', () => {
  ...
  describe('Admin', () => {
    ...
    it('should get customer fundings monitoring', async () => {
      ...
      expect(res.statusCode).toBe(200);
      expect(res.result.data[0]).toBeDefined();
      expect(res.result.data[0].plannedCareHours).toBe(40);
      expect(res.result.data[0][currentMonth]).toBe(6);
      expect(res.result.data[0][prevMonth]).toBe(4);
    });
  });
