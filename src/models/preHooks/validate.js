...
module.exports = {
  validateQuery(next) {
    ...
    const { isVendorUser, processingAuthentication, gettingOwnInfo } = this.getOptions();

    if (!hasCompany && !isPopulate && !isVendorUser && !processingAuthentication &&
      !gettingOwnInfo) next(Boom.badRequest());
    next();
  },
